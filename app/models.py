...
class GennisCapitalAssetTerm(Base):
    """Depreciation terms from old gennis's `capital` register.

    Not the same thing as GennisCapitalTerm, despite the similar name. That
    table's capital_id points at gennis_capital_expenditure and is generated by
    v2 itself (services/account_utils.py); these rows are terms against the
    `capital` register, which arrived here as gennis_capital_asset. Merging them
    would attach each row to whatever expenditure shares its id.
    """

    __tablename__ = "gennis_capital_asset_term"

    id                       = Column(BigInteger, primary_key=True, index=True)
    gennis_id                = Column(Integer, nullable=False, unique=True)
    capital_asset_gennis_id  = Column(Integer, nullable=True, index=True)
    calendar_month_gennis_id = Column(Integer, nullable=True)
    calendar_year_gennis_id  = Column(Integer, nullable=True)
    year                     = Column(Integer, nullable=True)
    month                    = Column(Integer, nullable=True)
    down_cost                = Column(Integer, nullable=True)
    account_period_id        = Column(Integer, nullable=True)
    synced_at                = Column(DateTime, server_default=func.now())


class GennisDeletedOverheadTypeLog(Base):
    """Soft-deleted overhead type logs from old gennis.

    overhead_type_log is live in v2 — a Celery task pre-creates a row per
    overhead type each month and the UI pays them off — so cancelled rows cannot
    go there without showing up as real unpaid overhead. Companion to
    gennis_deleted_overhead, which exists for the same reason.
    """

    __tablename__ = "gennis_deleted_overhead_type_log"

    id                       = Column(BigInteger, primary_key=True, index=True)
    gennis_id                = Column(Integer, nullable=False, unique=True)
    overhead_type_id         = Column(Integer, nullable=True, index=True)
    overhead_gennis_id       = Column(Integer, nullable=True)
    location_id              = Column(Integer, nullable=True, index=True)
    expected_amount          = Column(BigInteger, nullable=True)
    is_paid                  = Column(Boolean, nullable=True)
    is_prepaid               = Column(Boolean, nullable=True)
    paid_date                = Column(DateTime, nullable=True)
    calendar_month_gennis_id = Column(Integer, nullable=True)
    calendar_year_gennis_id  = Column(Integer, nullable=True)
    year                     = Column(Integer, nullable=True)
