...
const BRANCH_SITES: Record<string, { label: string; domain: string; path?: string }> = {
  ...
  // (SSO into it from here was tried via a token signed with this
  // project's SECRET_KEY, on the assumption gennis-v2 shared the same
  // key — it doesn't, in the real deployed .env files, so that token
  // never verifies there. Reverted to the same plain link-out every
  // other branch uses rather than fix the two secrets out of sync.)
